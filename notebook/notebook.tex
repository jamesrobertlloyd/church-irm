...
Data was complete social network and cold start tag data (think Facebook `likes') in the journal article version of AH paper (different fold at the moment).
...
First, product IRMs, would appear to be a poor model
...
Inspecting individual AUCs from each repeat showed that there was still a lot of variance.
...
Clearly the numbers are noisy, but they do not contradict the initial hypothesis that an additive model would be the better performer on this joint modelling task (since it has the capacity not to be distracted by 
...
2 minutes, 100 restarts, all folds.
What happens now?
